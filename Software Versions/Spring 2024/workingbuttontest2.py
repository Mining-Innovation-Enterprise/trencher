...
def accel_curve(value, motor): # ramps up the gantry motor speed to 2500
    ...
    avail_freq = [250,400,500,625,800,1000]#[800,1000,1250,2000,2500] # available frequncies for 2 uS, change if the sampling rate changes
    if motor == 'x':
        for z in range(len(avail_freq)):
            #print(z)
            if  (z<=(len(avail_freq)-2)):
              if((value>=avail_freq[z]) | (value<avail_freq[0]))& (value<avail_freq[z+1]):
                gantry_freq = avail_freq[z]
                #print(z)
                return gantry_freq
            elif (z==(len(avail_freq)-1)):
              gantry_freq = avail_freq[z]
              #print(z)
              return gantry_freq
...
while True:
   ...
   # duty = 0 #variable for duty cycle for BLDC
    ...
    if pi.read(x_plus) == 0: # drives the gantry clockwise
        ...
        pi.write(x_direction, 1)
        pi.set_PWM_dutycycle(x_step, 128)
        pi.set_PWM_frequency(x_step, x_freq)
